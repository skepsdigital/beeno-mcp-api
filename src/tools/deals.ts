...
export function registerDealTools(server: McpServer, client: BeenoApiClient): void {
  ...
  server.tool(
    ...
    'List deals with optional pagination, sorting and property selection',
    {
      ...paginationSchema,
      ...sortSchema,
      properties: z.string().optional().describe('Comma-separated list of properties to include'),
      includeAssociations: z.boolean().optional().describe('Whether to include associated records')
    },
    async (params) => {
      try {
  ...
  server.tool(
    ...
    'Create a new deal. Required properties typically include pipeline_id, stage_id, name.',
    {
      ...
      associations: z.object({
        ...
        })).optional().describe('Products to associate with the deal'),
        contacts: z.array(z.number()).optional().describe('Contact IDs to associate'),
        companies: z.array(z.number()).optional().describe('Company IDs to associate')
      }).optional().describe('Associations to create with the deal')
    },
    async (params) => {
      try {
        const body: Record<string, any> = { properties: params.properties };
        if (params.associations !== undefined) {
  ...
  server.tool(
    ...
    async (params) => {
      try {
        ...
        const result = await client.post('/deals/search', body);
        return { content: [{ type: 'text' as const, text: JSON.stringify(result, null, 2) }] };
      } catch (error: any) {
        return { content: [{ type: 'text' as const, text: `Error: ${error.message}` }], isError: true };
      }
  ...
  server.tool(
    ...
    'Replaces ALL products in the deal. Include existing products to keep them. Deal amount is recalculated automatically.',
    {
      ...
      products: z.array(z.object({
        productId: z.number().optional().describe('Product ID from library (for library products)'),
        ...
        name: z.string().optional().describe('Product name (required when isCustom=1)'),
        sku: z.string().optional().describe('Product SKU (optional)')
